...
import { CirclePlus, Ellipsis, FilePenLine, FileStack, FileText, Trash2 } from "lucide-react";
...
function MenuItem({item, handleDuplicar, handlePdf, handleDelete }: PropsMenuItem) {
    return (
        <DropdownMenu>
...
export default function ListaCvs() {
    ...
    return (
        <Page>
            <h1 className="text-center text-lg font-bold pb-4">Listado de CVs</h1>
                ...
                <tbody>
                    { data.map( (item,index) => (
                        <tr key={index}>
                            ...
                            <td className="text-center">{ format(item.created_at ?? '','medium','es-Pe') }</td>
                            <td className="text-center">{ format(item.updated_at ?? '','medium','es-Pe') }</td>
                        </tr>
                    ))}
                </tbody>
            </table>
